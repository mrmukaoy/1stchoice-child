...
function baselayer_child_get_sidebar_nav() {
	$sidebar = false;
...
function yo_baselayerposted_on() {
	...
	$svg = include( 'img/icon-cal-w-bg-svg.php' );

	$posted_on = sprintf(
		// translators: %s: post date.
		esc_html_x( '%s', 'post date', 'baselayer' ), $time_string 
	);

	echo '<span class="posted-on">' . $svg . $posted_on . '</span>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
}
	...
	if ( 'post' === get_post_type() ) {
		...
		*/
		...
		if ( !empty( $the_cats ) ) {
			...
			if ( 1 <= count($the_cats) ) {
				$i = 1;
				$categories_list = '<span class="cat-links">' . $svg;
				foreach( $the_cats as $cat ) {
		...
		if ( $tags_list ) {
			$svg = include( 'img/icon-tag-svg.php' );
